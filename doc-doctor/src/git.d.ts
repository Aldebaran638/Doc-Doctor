 ...
 * 远程仓库信息
 * 用途：描述 git remote 信息
 * 用法：repo.state.remotes 获取所有远程仓库
 */
 ...
 * 文件变更信息
 * 用途：描述工作区或暂存区的文件变更
 * 用法：repo.state.workingTreeChanges 获取未暂存变更
 */
export interface Change {
	/**
	 * 返回 `originalUri` 或 `renameUri`，取决于是否为重命名变更。
	 * 一般建议优先使用 `uri`。
	 */
	readonly uri: Uri;
...
/**
 * 仓库状态信息
 * 用途：描述当前仓库的所有状态，包括分支、远程、变更等
 * 用法：repo.state 获取仓库状态
 */
