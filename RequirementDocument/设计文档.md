...
graph TB
    subgraph "VS Code 环境"
        ...
        subgraph "扩展主体 (Extension Host)"
            EXT[extension.ts<br/>扩展入口]
            MSG --> |postMessage| EXT
            
            subgraph "服务层模块"
                PC[projectCheck.ts<br/>总检查模块]
                FC[fileCheck.ts<br/>文件解析模块]
                FN[functionCheck.ts<br/>函数检查模块]
                WL[fileWhiteList.ts<br/>白名单模块]
                JL[jumpToLocation.ts<br/>跳转模块]
            end
        ...
        subgraph "原生模块 (Native)"
            DB_CPP[doc_doctor_db.dll<br/>C++ 数据库模块]
            SQLITE[(SQLite3<br/>problems.db)]
            
            DB_TS --> |koffi FFI| DB_CPP
            DB_CPP --> SQLITE
        end
    end
    
    subgraph "配置文件"
        SETTINGS[.vscode/settings.json<br/>白名单配置]
        EXT -.-> |读取配置| SETTINGS
    end
```
...
graph LR
    ...
    DB --> |koffi| DLL[doc_doctor_db.dll]
    DLL --> SQLite[(SQLite3)]
...
sequenceDiagram
    ...
    participant FN as functionCheck
    participant WL as fileWhiteList
    participant DB as Database

    U->>W: 点击"检查整个项目"
    W->>E: postMessage({type: 'runProjectCheck'})
    E->>PC: runProjectCheck(webview)
    
    PC->>WL: getDocDoctorSettings()
    WL-->>PC: settings
    
    PC->>PC: findFiles('**/*.{c,cpp}')
    
    loop 遍历每个文件
        PC->>WL: isFileWhitelisted(filePath)
        alt 文件在白名单
            PC->>PC: 跳过文件
        else 文件不在白名单
            PC->>FC: checkFile(uri)
            FC-->>PC: ParseResult {functions}
            
            loop 遍历每个函数
                PC->>WL: shouldSkipFunction(func)
                alt 函数应跳过
                    PC->>PC: 跳过函数
                else 函数不跳过
                    PC->>FN: checkFunction(funcInfo)
                    FN-->>PC: problems[]
                    PC->>PC: 收集问题
                end
            end
    ...
    PC->>DB: saveProblemsToDBBatch(problems)
    DB-->>PC: SaveResult
...
sequenceDiagram
    ...
    participant W as Webview
    participant E as Extension
    participant JL as jumpToLocation
    participant VSC as VS Code Editor

    U->>W: 点击问题卡片
    W->>E: postMessage({type: 'jumpToProblem', data})
    E->>JL: jumpToLocation(filePath, line, col, funcName)
    
    JL->>JL: 解析文件路径
    JL->>VSC: workspace.fs.stat(fileUri)
    
    alt 文件存在
        JL->>VSC: openTextDocument(fileUri)
        JL->>VSC: showTextDocument(document)
...
sequenceDiagram
    ...
    U->>W: 修改设置并点击"保存"
    W->>E: postMessage({type: 'saveSettings', data})
    
    E->>CFG: config.update('checkMainFunction', value)
    E->>CFG: config.update('fileWhitelist', array)
    E->>CFG: config.update('functionWhitelist', object)
    E->>CFG: config.update('returnTypeWhitelist', array)
    
    CFG-->>E: 保存成功
    E->>W: postMessage({type: 'settingsSaved', success: true})
    
    Note over E,PC: 保存后自动触发重新检查
    E->>PC: runProjectCheck(webview)
```
