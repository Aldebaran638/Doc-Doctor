 ...
 */
export async function findRelatedFunctionsOnline(
  inputFunction: string,
  topK: number = 5
): Promise<RelatedFunction[]> {
  ...
  try {
    ...
    // 为所有候选函数生成向量嵌入并计算相似度
    const similarities: RelatedFunction[] = [];
    ...
    for (const func of mockFunctions) {
      ...
      const funcEmbedding = Array.from(funcResult.data as Float32Array);
